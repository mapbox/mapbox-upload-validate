...
module.exports.filepath = function validateFilepath(filepath, callback) {
  ...
  sniffer.quaff(filepath, true, function(err, protocol) {
    ...
    sniffer.quaff(filepath, false, function(err, filetype) {
      if (err) return callback(invalid(err));
...
module.exports.info = function validateInfo(results, limits, callback) {
  if (!callback) {
    callback = limits;
  ...
  if (results.filetype === 'kml') {
    var kmlValid = kmlValidator(results.filepath);
    if (kmlValid !== true) {
      return callback(invalid(kmlValid));
    }
  }

  tilelive.info(results.uri, function(err, info) {
    if (err) return callback(invalid(err));
