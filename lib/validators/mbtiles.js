...
module.exports = function validateMbtiles(opts, callback) {
  const limits = opts.limits || uploadLimits.mbtiles;
...
function validFormat(db, info, callback) {
  ...
  db.get('SELECT tile_data FROM tiles LIMIT 1', (err, result) => {
    err = sqliteError(err);
    ...
    const format = tiletype.type(result.tile_data);
    if (!format) return callback(invalid('Unknown format.'));

    if (format === 'pbf' && !('vector_layers' in info))
...
function deprecatedCarmen(db, callback) {
  ...
  db.get('SELECT COUNT(1) AS count FROM carmen', (err, result) => {
    err = sqliteError(err);
    if (err) return callback(err);
...
function zoomCheck(db, limits, callback) {
  ...
  queue()
    ...
    .awaitAll((err, zoom) => {
      ...
      const [tiles_max, grids_max, tiles_min, maxzoom, fillzoom] = zoom;
      ...
      if ('value' in fillzoom) {
        ...
        const fz = Number.parseInt(fillzoom.value, 10);
        if (fz < tiles_min.z) {
      ...
      if ('value' in maxzoom) {
        ...
        const mz = Number.parseInt(maxzoom.value, 10);
        if (mz < tiles_min.z) {
