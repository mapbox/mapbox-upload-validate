...
function validFormat(db, info, callback) {
  ...
  db.get('SELECT tile_data FROM tiles LIMIT 1', function(err, result) {
    ...
    if (format === 'pbf') {
      ...
      const ids = new Set();
      for (let i = 0; i < info.vector_layers.length; i++) {
        const id = info.vector_layers[i].id;
        if (ids.has(id)) return callback(invalid(`Vector source "vector_layers" contains duplicate ID: ${id}`));
        ids.add(info.vector_layers[i].id);
      }
    }
