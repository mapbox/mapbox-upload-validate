  ...
  queue()
    ...
    .awaitAll(function(err, zoom) {
      ...
      if ('z' in fillzoom) {
        var fz;
        try {
          fz = Number.parseInt(fillzoom.z, 10);
        } catch (e) {
          return callback(invalid('In mbtiles file, metadata.fillzoom is not numeric. Found: ' + fillzoom.z));
        }
        if (fz < tiles_min.z) {
      ...
      if ('z' in maxzoom) {
        var mz;
        try {
          mz = Number.parseInt(maxzoom.z, 10);
        } catch (e) {
          return callback(invalid('In mbtiles file, metadata.maxzoom is not numeric. Found: ' + maxzoom.z));
        }
        if (mz < tiles_min.z) {
