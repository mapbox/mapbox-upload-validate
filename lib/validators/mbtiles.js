...
function zoomCheck(db, limits, callback) {
  function getMaxZoom(type, cb) {
    ...
    switch (type) {
      ...
      case 'tiles_min':
        query = 'SELECT MIN(zoom_level) as z FROM grids';
        break;
      ...
      case 'fillzoom':
        query = 'SELECT (value) FROM metadata WHERE name=\'fillzoom\'';
        break;
    }
  ...
  queue()
    ...
    .awaitAll(function(err, zoom) {
      ...
      if ('value' in fillzoom) {
        var fz;
        try {
          fz = Number.parseInt(fillzoom.value, 10);
        } catch (e) {
          return callback(invalid('In mbtiles file, metadata.fillzoom is not numeric. Found: ' + fillzoom.value));
        }
        if (fz < tiles_min.z) {
      ...
      if ('value' in maxzoom) {
        var mz;
        try {
          mz = Number.parseInt(maxzoom.value, 10);
        } catch (e) {
          return callback(invalid('In mbtiles file, metadata.maxzoom is not numeric. Found: ' + maxzoom.value));
        }
        if (mz < tiles_min.z) {
