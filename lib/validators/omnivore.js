...
module.exports = function validateOmnivore(opts, callback) {
  ...
  sniffer.fromFile(opts.filepath, function(err, results) {
    if (err) return callback(err);

    if (results.type === 'tif') limits = opts.limits || uploadLimits.tif;
    else if (results.type === 'csv') limits = opts.limits || uploadLimits.csv;
    else if (results.type === 'geojson') limits = opts.limits || uploadLimits.geojson;
    else if (results.type === 'kml') limits = opts.limits || uploadLimits.kml;
