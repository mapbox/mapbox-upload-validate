...
module.exports = function validateOmnivore(opts, callback) {
  var limits = opts.limits || { max_filesize: 260 * 1024 * 1024 };
  ...
  q.awaitAll(function(err, stats) {
    ...
    if (size > limits.max_filesize) return callback(new Error('File is larger than ' + limits.max_filesize + ' bytes. Too big to process.'));
    callback();
  });
};
