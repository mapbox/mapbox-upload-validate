...
module.exports = function validateSerialtiles(opts, callback) {
  ...
  read.pipe(zlib.createGunzip())
    ...
    .on('tile', function(t) {
      ...
      if (!gzipped && (format !== 'png' && format !== 'webp' && format !== 'jpg')) return callback(invalid('Invalid tiletype'));

      if (gzipped) {
        //gunzip buffer and validate
        validateGzipped(t, function(err) {
          if (err) return callback(err);
          console.log(t.buffer.length);
          if (t.buffer.length <= limits.max_tilesize) return;
          else return callback(invalid('Tile exceeds maximum size of ' + Math.round(limits.max_tilesize/1024) + 'k at z' + t.z + '. Reduce the detail of data at this zoom level or omit it by adjusting your minzoom.'));
          errored = true;
          read.close();
        });
  ...
  function validateGzipped(tile, callback) {
    ...
    zlib.gunzip(tile.buffer, function(err, data) { 
      ...
      vtile.setData(data, function(err) {
        ...
        vtile.parse();
        if (vtile.empty()) return callback(invalid('Empty tile'));
        //if (vtile.names()) return callback(invalid('Tile has no layers'));
        if (!vtile.toJSON()[0].name) return callback(invalid('Tile has no layers'));
        if (!vtile.toJSON()[0].features) return callback(invalid('Tile has no features'));
