...
module.exports = function validateSerialtiles(opts, callback) {
  ...
  read.pipe(zlib.createGunzip())
    ...
    .on('tile', function(t) {
      ...
      console.log("header: " + JSON.stringify(tiletype.headers(t.buffer)));
  ...
  function validateGzipped(tile, callback) {
    console.log("in validateGzipped!!!!!!!");
    ...
    zlib.gunzip(tile.buffer, function(err, data) { 
      vtile.setData(data, function(err) {
        ...
        if (vtile.names().length < 1) return callback(invalid('Tile has no layers'));
        console.log(vtile.toJSON());
        console.log(vtile.toJSON()[0].features);
