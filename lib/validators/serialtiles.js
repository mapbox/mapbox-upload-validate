...
module.exports = function validateSerialtiles(opts, callback) {
  ...
  read.pipe(zlib.createGunzip())
    ...
    .on('tile', function(t) {
      ...
      callback(new Error('Tile exceeds maximum size of ' + Math.round(limits.max_tilesize/1024) + 'k at z' + t.z + '. Reduce the detail of data at this zoom level or omit it by adjusting your minzoom.'));
      errored = true;
      read.close();
    })
