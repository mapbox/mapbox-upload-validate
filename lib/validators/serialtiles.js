...
module.exports = function validateSerialtiles(opts, callback) {
  ...
  var max_tilesize = opts.max_tilesize || (1024 * 500);
  var errored = false;
  ...
  read.pipe(zlib.createGunzip())
    ...
    .on('tile', function(t) {
      if (errored) return;
      if (!t.buffer) return;
      if (t.buffer.length <= max_tilesize) return;
      callback(new Error('Tile exceeds maximum size of ' + Math.round(max_tilesize/1024) + 'k at z' + t.z + '. Reduce the detail of data at this zoom level or omit it by adjusting your minzoom.'));
      errored = true;
      read.close();
    })
